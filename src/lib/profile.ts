...
export function profileFromUser(
    user: Pick<User, 'user_metadata' | 'email' | 'new_email'>,
): Profile {
    const meta = user.user_metadata ?? {};
    const saved = meta.mastercheck_profile ?? {};
    const parts = text(meta.full_name || meta.name)
        .trim()
        .split(/\s+/)
        .filter(Boolean);
    return {
...
export function validateProfile(profile: Profile): string | null {
    ...
    if (profile.firstName.trim().length > 80 || profile.lastName.trim().length > 80)
        return 'Ім’я та прізвище — до 80 символів кожне.';
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(profile.email.trim()) || profile.email.length > 254)
        return 'Вкажіть коректну електронну пошту.';
    if (
        profile.phone &&
        (!/^[+\d\s().-]+$/.test(profile.phone) ||
            profile.phone.replace(/\D/g, '').length < 7 ||
            profile.phone.length > 30)
    )
