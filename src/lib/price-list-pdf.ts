...
export function buildPriceListPdf(
    ...
    doc.setProperties({
        ...
        creator: 'Мій кошторис',
    });
    const categoryNames = new Map(data.categories.map((category) => [category.id, category.name]));
    const date = [issuedAt.getDate(), issuedAt.getMonth() + 1, issuedAt.getFullYear()]
        .map((part) => String(part).padStart(2, '0'))
        .join('.');
    doc.setFontSize(11);
    const authorLines: string[] = doc.splitTextToSize(cleanText(data.author).slice(0, 160), 176);
    doc.setFontSize(9);
    const contactLines: string[] = data.contact.trim()
        ? doc.splitTextToSize(cleanText(data.contact).slice(0, 160), 176)
        : [];
    ...
    autoTable(doc, {
        ...
        body: data.services.map((service) => [
            ...
            cleanText(categoryNames.get(service.category_id) ?? 'Без категорії'),
    ...
    for (let page = 1; page <= pages; page++) {
        ...
        if (contactLines.length)
            doc.text(contactLines, 17, 32 + authorLines.length * 5, { lineHeightFactor: 1.2 });
        doc.setFontSize(8);
