export const serviceUnits = {
    m2: 'м²', lm: 'пог. м', m3: 'м³', piece: 'шт.', hour: 'год',
    day: 'день', set: 'комплект', service: 'послуга', kg: 'кг', tonne: 'т',
} as const;
...
export type Service = {
    id: string; user_id: string; category_id: string; name: string;
    price: number; unit: ServiceUnit; created_at: string;
};
...
export function selectServices(services: Service[], categories: ServiceCategory[], categoryId: string, query: string, sort: ServiceSort) {
    const names = new Map(categories.map((category) => [category.id, category.name]));
    const search = query.trim().toLocaleLowerCase('uk-UA');
    const compare = (a: string, b: string) => a.localeCompare(b, 'uk-UA');
    return services.filter((service) => (!categoryId || service.category_id === categoryId)
        && `${service.name} ${names.get(service.category_id) ?? ''}`.toLocaleLowerCase('uk-UA').includes(search))
        .sort((a, b) => {
            ...
            return compare(names.get(a.category_id) ?? '', names.get(b.category_id) ?? '') || byName;
        });
}
...
export function validateService(name: string, price: string, unit: string, categoryId: string) {
    ...
    if (!Object.prototype.hasOwnProperty.call(serviceUnits, unit)) return 'Оберіть одиницю виміру зі списку.';
...
export function priceListError(error: { code?: string }) {
    if (error.code === 'PGRST205' || error.code === '42P01') return 'Прайс-лист ще не налаштований у базі. Зверніться до адміністратора застосунку.';
    if (error.code === '23505') return 'Категорія з такою назвою вже існує.';
    if (error.code === '23503') return 'Категорія змінилася або містить послуги. Оновіть список; перед видаленням категорії перенесіть її послуги.';
    if (error.code === '42501' || error.code === 'PGRST116') return 'Запис недоступний. Оновіть список або увійдіть повторно.';
